...
| `max_continuation_context_bytes` | Covered | `fake_inline_continuation_context_bytes_limit_exceeded`; `http_inline_continuation_context_bytes_limit_exceeded` |
...
| `transaction_deadline` | Covered | `transaction_limits_transaction_deadline_hang_ends_deadline_exceeded` (sets `TransactionLimits.transaction_deadline` on `StartedRuntime`; Hang exchange → `DeadlineExceeded`) |
| `cleanup_deadline` | Partial | Wired to exchange `children.wait(cleanup_deadline)` and quiesce hard-grace (`cleanup_deadline.max(2s)` floor on that path only). No distinct fail-closed completion code / exact-admit cell yet |
...
- Race/load beyond named Fake proofs (inventory: `doc/S23_RACE_LOAD_INVENTORY.md`; still not exhaustive / not live Grok)
