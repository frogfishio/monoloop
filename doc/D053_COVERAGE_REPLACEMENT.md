...
| `tests/direct_llm_e2e.rs` | DirectLlm FakeConnector + HTTP/OpenAI SSE composition, continuation, tool second-exchange, call-ID reuse, concurrency | **Phase A+B partial replacement:** `tests/direct_llm_openai_e2e.rs` — HTTP/OpenAI text-only + concurrent admits; CallerControlled tool path encodes admitted tools, dispatches Ready, ends `ContinuationRequired` with **one** HTTP open (`caller_controlled_tool_exchange_ends_continuation_required_without_second_open`). Retained Fake smoke unchanged. **Still open Golden residual:** inline model/tool/model (`encode_tool_continuation`), call-ID reuse across exchanges e2e. |
