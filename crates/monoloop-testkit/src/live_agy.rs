//! End-to-end live Antigravity ACP driver (via `agy-acp` bridge).
//!
//! **Test kit only.** Requires `agy` authenticated and an ACP bridge
//! (`agy-acp` on PATH or `npx agy-acp`).
...
use monoloop_connector_agy::{AgyAgentConfig, AgyAgentHandle, AgySessionConfig};
...
impl LiveAgyRunOptions {
    /// Defaults under `target/live_agy_run`.
    pub fn for_project(project: impl Into<PathBuf>, prompt: impl Into<String>) -> Self {
        let project = project.into();
        let stem = project.join("target/live_agy_run");
        let mut agent = AgyAgentConfig::for_project(project.clone());
        ...
        Self {
            ...
            cwd: project.clone(),
            agent,
            session: AgySessionConfig::new(project),
            title: "Live Antigravity ACP — interpretation review".into(),
            artifact_stem: stem,
            render_console: true,
...
pub async fn run_live_agy_prompt(opts: LiveAgyRunOptions) -> Result<LiveAgyRunReport, String> {
    if let Some(parent) = opts.artifact_stem.parent() {
        std::fs::create_dir_all(parent).map_err(|e| e.to_string())?;
    }

    let mut agent = AgyAgentHandle::connect(opts.agent.clone())
        .await
    ...
    let interp = factory
        .start(StartInterpretation {
            ...
            connection_id: monoloop_contracts::ConnectionId::new("agy-live"),
    ...
    let mut sequence_text = String::from("=== LIVE AGY — CANONICAL TEXT ===\n");
