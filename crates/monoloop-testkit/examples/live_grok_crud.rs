...
use monoloop_connector_grok::{
    ...
    InMemorySecretResolver, RawDumpCollector, SecretRef,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = GrokServerConfig::loopback(port, SecretRef::new("GROK_WS_SECRET"))?
    ...
    // Long timeout: real model + tools.
    let result = tokio::time::timeout(Duration::from_secs(300), exchange.response)
        .await
        .map_err(|_| "session/prompt timed out (5 min)")?
        .map_err(|_| "prompt response channel dropped")??;
