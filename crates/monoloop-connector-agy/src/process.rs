//! Antigravity ACP process: NDJSON JSON-RPC over stdio (`agy-acp` or native).

use crate::config::AgyAgentConfig;
...
pub(crate) struct ProcessInner {
    pub config: AgyAgentConfig,
    next_id: AtomicU64,
    stdin: Mutex<Option<ChildStdin>>,
    pending: Mutex<HashMap<u64, oneshot::Sender<Result<Value, AgyConnectorError>>>>,
...
impl ProcessInner {
    pub async fn spawn(
        ...
        dump: Arc<AgyRawDump>,
    ) -> Result<Arc<Self>, AgyConnectorError> {
        ...
        let mut child = cmd.spawn().map_err(|e| {
            AgyConnectorError::connection(format!(
                "failed to spawn agy ACP server ({} {:?}): {e}",
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| AgyConnectorError::connection("agent stdin missing"))?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| AgyConnectorError::connection("agent stdout missing"))?;
                ...
                if let Some(tx) = pending.remove(&id) {
                    if let Some(err) = value.get("error") {
                        let msg = err
                            ...
                            .and_then(|m| m.as_str())
                            .unwrap_or("rpc error");
                        let _ = tx.send(Err(AgyConnectorError::protocol(msg.to_string())));
                    } else {
            ...
            if let Some(id) = value.get("id") {
                ...
                if method.starts_with("session/") || method.starts_with("cursor/") {
                    let resp = serde_json::json!({
                        ...
                        "error": {
                            "code": -32601,
                            "message": format!("method not handled by monoloop agy connector: {method}")
                        }
                    });
                    let _ = self.write_value(&resp).await;
                    return;
    ...
    async fn write_value(&self, value: &Value) -> Result<(), AgyConnectorError> {
        let line = serde_json::to_string(value)
            .map_err(|e| AgyConnectorError::protocol(format!("encode: {e}")))?;
        self.dump.push_line(format!(">> {line}"));
        let mut guard = self.stdin.lock().await;
        let stdin = guard
            .as_mut()
            .ok_or_else(|| AgyConnectorError::connection("stdin closed"))?;
        stdin
            .write_all(line.as_bytes())
            .await
            .map_err(|e| AgyConnectorError::connection(format!("write: {e}")))?;
        stdin
            .write_all(b"\n")
            .await
            .map_err(|e| AgyConnectorError::connection(format!("write nl: {e}")))?;
        stdin
            .flush()
            .await
            .map_err(|e| AgyConnectorError::connection(format!("flush: {e}")))?;
        Ok(())
    }
    ...
    pub async fn request(&self, method: &str, params: Value) -> Result<Value, AgyConnectorError> {
        ...
        match tokio::time::timeout(self.config.rpc_deadline, rx).await {
            Ok(Ok(r)) => r,
            Ok(Err(_)) => Err(AgyConnectorError::connection("response channel dropped")),
            Err(_) => {
                let mut pending = self.pending.lock().await;
                pending.remove(&id);
                Err(AgyConnectorError::deadline(format!(
    ...
    async fn fail_all_pending(&self, err: AgyConnectorError) {
        let mut pending = self.pending.lock().await;
...
async fn read_line_bounded<R: AsyncBufReadExt + Unpin>(
    reader: &mut R,
    max_bytes: usize,
) -> Result<Option<String>, AgyConnectorError> {
    ...
    loop {
        let available = reader
            .fill_buf()
            .await
            .map_err(|e| AgyConnectorError::connection(format!("stdout read: {e}")))?;
        ...
        if let Some(pos) = available.iter().position(|&b| b == b'\n') {
            ...
            if buf.len() + take > max_bytes {
                reader.consume(take);
                return Err(AgyConnectorError::protocol(
