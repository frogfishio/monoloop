...
async fn run_ndjson(lines: &[&str]) -> Vec<InterpreterOutputEvent> {
    ...
    let interp = factory
        .start(StartInterpretation {
            interpretation_id: InterpretationId::new("agy-i1"),
            connection_id: ConnectionId::new("agy-c1"),
            external_session_id: Some(monoloop_contracts::ExternalSessionId::new("sess-agy-1")),
            dialect: agy_dialect(),
...
#[tokio::test]
async fn agy_message_chunks_assemble_sentence() {
    // Shape observed from agy-acp live probe (messageId optional field).
    let lines = [
...
#[tokio::test]
async fn agy_tool_call_lifecycle() {
    let lines = [
